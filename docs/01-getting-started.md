...
- `COLD_START=true` → يبدأ بدون أي بيانات ومفيش أي صنف (catalogue فاضي): مفيش توقّع لحد ما
  تبعتي بيانات، والكاش katalog بيتعلّم من اللي بترفعيه بس — دي طريقة الاختبار «على DB فاضي».
  `POST /catalogue/upsert` لتسجيل الأصناف (سعر/تكلفة/صلاحية) قبل أو مع المبيعات.
  - `REGISTRY_STORE=data/registry.json` → يحفظ ما تعلّمه لكل مطعم بعد الريستارت.

> ملاحظة: بدون `COLD_START=true`، السيرفر بيتدرّب على الداتا المحاكاة كاملة (11 صنف موجودة
> في الداتا نفسها، مش من كود). علشان تجربة مخبز جديد حقيقي (صفر أصناف) استخدمي `COLD_START=true`.
...
│   │   ├── items.py              ← الـ 11 صنف = fixtures للمحاكاة بس. الكتالوج الحقيقي
│   │   │                            ديناميكي (Catalogue): بيتجمّع من اللي بترفعيه (سعر/تكلفة/صلاحية → newsvendor q*)

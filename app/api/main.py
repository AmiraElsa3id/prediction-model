...
async def lifespan(app: FastAPI):
    ...
    # Per-restaurant learned levels. MONGO_URL, when set, wins -- a real database
    # survives a restart AND works across more than one instance of this service,
    # unlike the JSON-file fallback (one file on one machine's disk). Falls back to
    # the JSON file when MONGO_URL is unset, and to pure in-memory when
    # REGISTRY_STORE is explicitly "" (what the test suite does to stay isolated).
    mongo_url = os.getenv("MONGO_URL")
    store = os.getenv("REGISTRY_STORE", "data/registry.json")
    STATE["registry"] = RestaurantRegistry(
        persist_path=None if mongo_url else (store or None),
        mongo_url=mongo_url,
    )
    # Market research agent (plan.md Part C). `available` is False without both
    # TAVILY_API_KEY and LLM_API_KEY configured -- every call degrades to a per-pair
    # "unavailable" result rather than the endpoint failing outright.
    STATE["research_agent"] = MarketResearchAgent()
    # Storage for research results needs Mongo specifically -- researched priors are
    # inherently per-tenant and queryable, the JSON-file registry mode has no
    # equivalent for this (see registry.py's _priors_store).
    if mongo_url:
        from app.integration.priors_store import PriorsStore

        try:
            STATE["priors_store"] = PriorsStore(mongo_url)
        except Exception:
            STATE["priors_store"] = None
    else:
        STATE["priors_store"] = None
...
@app.post(
    "/integration/restomind/research-priors",
    response_model=schemas.ResearchPriorsResponse,
    tags=["restomind"],
)
def research_priors(req: schemas.ResearchPriorsRequest) -> schemas.ResearchPriorsResponse:
    """Search real sources for how each category actually behaves around each
    calendar event, and ask the LLM to estimate a multiplier FROM those sources.

    Every successfully-researched result is stored as `pending_review` -- it does
    NOT affect any forecast until a separate, explicit call to
    /integration/restomind/approve-priors. See app/agents/market_research.py's and
    app/integration/priors_store.py's module docstrings for the full reasoning.
    """
    agent: MarketResearchAgent = STATE["research_agent"]
    results = agent.research_categories(req.categories, req.events, req.location)

    store = STATE.get("priors_store")
    if store is not None:
        for result in results:
            store.save_researched(req.restaurantId, result)

    return schemas.ResearchPriorsResponse(restaurantId=req.restaurantId, results=results)


@app.get(
    "/integration/restomind/pending-priors/{restaurant_id}",
    response_model=schemas.PendingPriorsResponse,
    tags=["restomind"],
)
def pending_priors(restaurant_id: str) -> schemas.PendingPriorsResponse:
    """Everything researched but not yet approved for this restaurant -- what a
    manager needs to see to make the approve/reject call themselves."""
    store = STATE.get("priors_store")
    pending = store.list_pending(restaurant_id) if store is not None else []
    return schemas.PendingPriorsResponse(restaurantId=restaurant_id, pending=pending)


@app.post(
    "/integration/restomind/approve-priors",
    response_model=schemas.ApprovePriorsResponse,
    tags=["restomind"],
)
def approve_priors(req: schemas.ApprovePriorsRequest) -> schemas.ApprovePriorsResponse:
    """The only thing that moves a researched multiplier into the live forecast
    path -- a human (or an explicit automated policy, but never the agent itself)
    reviewing and accepting one specific (category, event) result."""
    store = STATE.get("priors_store")
    if store is None:
        return schemas.ApprovePriorsResponse(
            approved=False, message="No MONGO_URL configured -- nothing to approve."
        )
    ok = store.approve(req.restaurantId, req.category, req.event)
    return schemas.ApprovePriorsResponse(
        approved=ok,
        message="Approved -- live forecasts for this category will use it." if ok
        else "No pending research found for that restaurant/category/event.",
    )
